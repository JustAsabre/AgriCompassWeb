...
try {
  ...
  console.warn("@replit/vite-plugin-runtime-error-modal not available during Vite config load; continuing without it.", err && err.message ? err.message : String(err));
}
...
try {
  ...
  console.warn("@vitejs/plugin-react not available during Vite config load; continuing without it.", err && err.message ? err.message : String(err));
}
